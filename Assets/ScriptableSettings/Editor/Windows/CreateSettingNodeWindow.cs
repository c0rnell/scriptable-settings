...
namespace Scriptable.Settings.Editor
{
    public class CreateSettingNodeWindow : EditorWindow
    {
            ...
            Action<ScriptableObject> addExistingCallback)
        {
            ...
            window.onCreateConfirmed = createCallback;
            window.onAddExistingConfirmed = addExistingCallback;
            window._parent = nodeParent;

            // Adjust size if needed to accommodate tabs
            window.ShowAsDropDown(GUIUtility.GUIToScreenRect(positionParent.worldBound), new Vector2(400, 250)); // Increased height
        }
